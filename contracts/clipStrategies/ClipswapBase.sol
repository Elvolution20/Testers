...
// Base contract to be inherited, should work with clipswap MasterChef:
...
contract ClipswapBase is Initializable, UUPSUpgradeable, OwnableUpgradeable, IStrategy {
    ...
    */
    ...
    // ERC20 internal immutable clip; 
    // IBiswapFarm internal immutable farm;
    // IUniswapV2Router02 internal immutable clipswapRouter;
    ...
    */
    constructor(Parameters memory _params) {
        ...
        // clip = ERC20(0x965F527D9159dCe6288a2219DB51fc6Eef120dD1);
        // farm = IBiswapFarm(0xDbc1A13490deeF9c3C12b44FE77b503c1B061739);
        // clipswapRouter = IUniswapV2Router02(0x3a6d8cA21D1CF76F653A67577FA0D27453350dD8);
    ...
    function deposit(uint256 amount) external override onlyOwner {
        ...
        _p.tokenA.approve(address(_p.clipswapRouter), amountA);
        _p.tokenB.approve(address(_p.clipswapRouter), amountB);
        (, , uint256 liquidity) = _p.clipswapRouter.addLiquidity(
            address(_p.tokenA),
            address(_p.tokenB),
            amountA,
            amountB,
        ...
        returns (uint256 amountWithdrawn)
    {
        ...
        amountB = _p.clipswapRouter.quote(amountB, balance0, balance1);
        ...
        _p.lpToken.approve(address(_p.clipswapRouter), liquidityToRemove);
        (amountA, amountB) = _p.clipswapRouter.removeLiquidity(
            address(_p.tokenA),
            address(_p.tokenB),
    ...
    function compound() external override onlyOwner {
        ...
        if (clipAmount > 0) {
            ...
            tokenA.approve(address(_p.clipswapRouter), balanceA);
            tokenB.approve(address(_p.clipswapRouter), balanceB);

            _p.clipswapRouter.addLiquidity(
                address(_p.tokenA),
                address(_p.tokenB),
                balanceA,
                balanceB,
    ...
    function totalTokens() external view override returns (uint256) {
        ...
        if (amountB > 0) {
            ...
            amountA += _p.clipswapRouter.quote(amountB, _reserve0, _reserve1);
        }
    ...
    function withdrawAll() external override onlyOwner returns (uint256 amountWithdrawn) {
        ...
        if (amount > 0) {
            ...
            uint256 lpAmount = _p.lpToken.balanceOf(address(this));
            _p.lpToken.approve(address(_p.clipswapRouter), lpAmount);
            _p.clipswapRouter.removeLiquidity(
                address(_p.tokenA),
                address(_p.tokenB),
                _p.lpToken.balanceOf(address(this)),
                0,
