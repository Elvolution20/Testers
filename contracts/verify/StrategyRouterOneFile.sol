...
contract Batch is Ownable {
    ...
    StrategyRouterOnefile public router;
...
library StrategyRouterLib {
    ...
    function getStrategiesValue(StrategyInfo[] storage self, IUsdOracle oracle)
        public
        view
        returns (uint256 totalBalance, uint256[] memory balances)
    {
        balances = new uint256[](self.length);
        for (uint256 i; i < balances.length; i++) {
            address token = self[i].depositToken;

            uint256 balanceInDepositToken = IStrategy(self[i].strategyAddress).totalTokens();
    ...
    function calculateSharesFromReceipt(
        mapping(uint256 => Cycle) storage self,
        uint256 receiptId,
        ...
        uint256 depositCycleTokenPriceUsd = self[receipt.cycleId].prices[receipt.token];
        ...
        uint256 allocatedUsdValueOfReceipt = (depositedUsdValueOfReceipt *
            self[receipt.cycleId].receivedByStrategiesInUsd) / self[receipt.cycleId].totalDepositedInUsd;
        return (allocatedUsdValueOfReceipt * PRECISION) / self[receipt.cycleId].pricePerShare;
    }
    ...
    function redeemReceiptsToSharesByModerators(
        ...
        for (uint256 i = 0; i < receiptIds.length; i++) {
            uint256 receiptId = receiptIds[i];
            uint256 shares = calculateSharesFromReceipt(self, receiptId, _currentCycleId, _receiptContract);
    ...
    function burnReceipts(
        ...
        for (uint256 i = 0; i < receiptIds.length; i++) {
            uint256 receiptId = receiptIds[i];
            if (_receiptContract.ownerOf(receiptId) != msg.sender) revert StrategyRouterOnefile.NotReceiptOwner();
    ...
    function getStrategyPercentWeight(StrategyInfo[] storage self, uint256 _strategyId)
        internal
        view
        returns (uint256 strategyPercentAllocation)
    {
        ...
        for (uint256 i; i < len; i++) {
            totalStrategyWeight += self[i].weight;
        }
        strategyPercentAllocation = (self[_strategyId].weight * PRECISION) / totalStrategyWeight;
        ...
        returns (uint256[] memory balances)
    {
        ...
        uint256 len = self.length;
        if (len < 2) revert StrategyRouterOnefile.NothingToRebalance();
        ...
        for (uint256 i; i < len; i++) {
            _strategiesTokens[i] = self[i].depositToken;
            _strategies[i] = self[i].strategyAddress;
        ...
        for (uint256 i; i < len; i++) {
            uint256 desiredBalance = (totalBalance * getStrategyPercentWeight(self, i)) / PRECISION;
...
contract StrategyRouterOnefile is Ownable {
    /* EVENTS */
