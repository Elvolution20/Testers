...
let weth, factory, uniswapRouter, mockFeed;
...
describe("Test Dodostrategy", function () {
  before(async function () {
    ...
    await setupCore();
    fakeToken1 = await deploy("Token");
    fakeToken2 = await deploy("Token");
    fakeToken3 = await deploy("Token");
    fakeToken4 = await deploy("Token");
    fakePriceFeed = await getMockFeed();
    fakePriceFeed2 = await getMockFeed();
    // setupFakePrices();
    // setupTestParams();
  ...
  describe("Test rebalanceBatch function", function () {
    it("usdt strategy, router supports only usdt, should revert", async function () {
      // await router.topUp({value: value});(10000).toString()
      strategy1 = await deployFakeStrategy(
        usdt.address, 
        async function() {
    ...
    it("should setPriceFeeds store feed address", async function () {
      tokens = [fakeToken1.address, fakeToken2.address];
      feeds = [fakePriceFeed.address, fakePriceFeed2.address];
    ...
    it("should getAssetUsdPrice revert on stale price", async function () {
      const _mockFeed = await getMockFeed();

      let tokens = [fakeToken1.address];
      let feeds = [_mockFeed.address];
      await oracle.setPriceFeeds(tokens, feeds);

      // roundId = 0, answer = 0, startedAt = 0, updatedAt = 0, answeredInRound = 0
      await _mockFeed.mock.latestRoundData.returns(0, 0, 0, 0, 0);
      await expect(
    ...
    it("should getAssetUsdPrice revert on 0 price", async function () {
      mockFeed = await getMockFeed();

      let _tokens = [fakeToken1.address];
      let _feeds = [mockFeed.address];
      await oracle.setPriceFeeds(_tokens, _feeds);
    ...
    it("usdt strategy, router supports multiple arbitrary tokens", async function () {
      ...
      strategy2 = await deployFakeStrategy(
        busd.address,
        async function() {
      ...
      strategy3 = await deployFakeStrategy(
        usdc.address,
        async function() {
  ...
  async function setupCore() {
    ...
    oracle = await deploy("ChainlinkOracleOnefile");
  ...
  async function deployFakeStrategy(tokenAddress, _callback) {
    // console.log(router.address, await token.name(), weight, profitPercent);
    const weight = 10_000;
    const profitPercent = 10_000;

    let _strategy = await deploy("MockStrategy", usdt.address, profitPercent);
    await _strategy.transferOwnership(router.address);
    await _callback();
    await router.addStrategy(_strategy.address, tokenAddress, weight);
    return _strategy;
  }
  ...
  // async function createMockStrategy(depositToken) {
  //   const  profit_percent = 10_000;
  //   const farm = await deploy("MockStrategy", usdt.address, profitPercent);
  //   return farm;
  // }
