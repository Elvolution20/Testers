...
//   function getStrategiesCount() external view returns (uint256 count);
//   function getStrategyDepositToken(uint256 i) external view returns (address);
//   function getStrategyPercentWeight(uint256 _strategyId) external view returns (uint256 strategyPercentAllocation);
...
//     event SetAddresses(Exchange _exchange, IUsdOracle _oracle, IStrategy _router, ReceiptNFT _receiptNft);
...
//         emit SetAddresses(_exchange, _oracle, IStrategy(_router), _receiptNft);
//     }
...
//         // point 4
//         uint256 strategiesCount = IStrategyRouter(router).getStrategiesCount();
...
//             address depositToken = IStrategyRouter(router).getStrategyDepositToken(i);
...
//         for (uint256 i; i < strategiesCount; i++) {
//             uint256 desiredBalance = (totalInBatch * IStrategyRouter(router).getStrategyPercentWeight(i)) / 1e18;
//             desiredBalance = fromUniform(desiredBalance, IStrategyRouter(router).getStrategyDepositToken(i));
...
//                         : IStrategyRouter(router).getStrategyDepositToken(i);
//                     address buyToken = IStrategyRouter(router).getStrategyDepositToken(j);
...
//         } else {
//             uint8 len = uint8(IStrategyRouter(router).getStrategiesCount());
//             // don't remove tokens that are in use by active strategies
//             for (uint256 i = 0; i < len; i++) {
//                 if (IStrategyRouter(router).getStrategyDepositToken(i) == tokenAddress) {
...
//     function getStrategiesValue(IUsdOracle oracle, IStrategy.StrategyInfo[] storage strategies)
//         public
//         view
...
//         mapping(uint256 => IStrategy.Cycle) storage cycles
...
//         for (uint256 i = 0; i < receiptIds.length; i++) {
//             uint256 receiptId = receiptIds[i];
//             if (_receiptContract.ownerOf(receiptId) != msg.sender) revert IStrategy.NotReceiptOwner();
...
//     function getStrategyPercentWeight(uint256 _strategyId, IStrategy.StrategyInfo[] storage strategies)
//         internal
//         view
...
//     function rebalanceStrategies(Exchange exchange, IStrategy.StrategyInfo[] storage strategies)
//         public
...
//         uint256 len = strategies.length;
//         if (len < 2) revert IStrategy.NothingToRebalance();
...
// contract StrategyRouterOnefile is IStrategyRouter, Moderator {
//     /* EVENTS */
